...
// 264×40 RGBA image at a non-zero origin (3, 3). Width 264 is an exact
// multiple of blockSize 8, so no padding column is needed.
...
// BenchmarkBlockAverage_Pixelate_Padded benchmarks the full pixelation pass on
// a 261×40 image (blockSize 8, remainder 3) where three columns of white
// padding are needed — the case that exercises the FillWhite padding path.
func BenchmarkBlockAverage_Pixelate_Padded(b *testing.B) {
	const w, h = 261, 40
