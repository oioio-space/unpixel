...
**Non livré — caveat structural** :
- [ ] **#2 (HMM émissions k-means/Viterbi complet)** : demande observations **colonne-ancrées aveugles** (restructure le pipeline offset-découverte/recherche) ; `window-hmm` est le variant beam (analogue au mono-hmm monospace). Récupération images **réelles reste font-boundée** sur tous les décodeurs ; fournir la police exacte via `--font`.
- [ ] **#6 (benchmark SICK/MICR parity)** : not done.
