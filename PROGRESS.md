...
**(A) Décodage multi-frame automatique.** Paquet `mosaictext` + CLI/MCP `--frame a.png --frame b.png …` (répétable) remplacent le « fuse-then-decode » par generate-and-test scorant chaque candidat contre **tous** les frames (découverte automatique de phase par frame, sans l'appelant). `mosaictext.DecodeMultiFrameAuto(ctx, imgs []image.Image, opts...)` : auto-détecte les phases grille par luma-variance (DiscoverPhases) et score chaque candidat contre **chaque frame à sa phase** (moyenne des distances — pas de fusion d'image), gardant le meilleur. MCP `unpixel_decode` `method: multi-frame` accepte `frame_offsets: [0, 0, 0]` → auto-détection quand tous 0. **Caveat honnête (mesuré `mfmeasure`)** : le scoring par moyenne n'**améliore PAS** la récupération sur le corpus synthétique (3/12 régimes améliorent = bruit, 4/12 dégradent ; la moyenne est monotone, elle ne crée pas de signal absent). L'apport réel est confiné au régime *sample-starved* (bloc ≥ largeur de glyphe, ex. IBAN bloc 50 de Positive Security) que nos fixtures à calibration aveugle ne peuvent pas exercer — frontière documentée, pas un gain livré. Ce qui EST livré : une API multi-frame **correcte** (retourne un décodage préservant la qualité single-frame au lieu de l'ancien `ErrNoMosaic` quand la fusion marchait) + découverte de phase auto. Panel 17/17 byte-identical (single-frame == plain Decode).
