...
//	docs/GEOMETRY.md                        human-readable (appended)
//
// Run with:
//
//	mise run geomeasure
...
func TestGeomeasure(t *testing.T) {
	...
	appendGeomMarkdown(t, stamp, results)
}
...
func appendGeomMarkdown(t *testing.T, stamp string, results []geomResult) {
	t.Helper()
	...
	mdf, err := os.OpenFile(path, os.O_CREATE|os.O_APPEND|os.O_WRONLY, 0o644)
	if err != nil {
		t.Fatalf("open %s: %v", path, err)
	}
	defer mdf.Close()
	if _, err := mdf.WriteString(sb.String()); err != nil {
		t.Fatalf("write %s: %v", path, err)
	}
	t.Logf("Markdown appended → %s", path)
}
